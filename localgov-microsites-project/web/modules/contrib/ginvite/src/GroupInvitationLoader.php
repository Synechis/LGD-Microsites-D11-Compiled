...
/**
 * Loader for wrapped 'group_invitation' group relationships.
 */
class GroupInvitationLoader implements GroupInvitationLoaderInterface {
   ...
   *   User.
   *
   * @return array|\Drupal\ginvite\GroupInvitation[]
   *   Group invitations
   */
  public function getUserInvitations(AccountInterface $user) {
    $cid = GroupInvitationLoaderInterface::GINVITE_USER_INVITATIONS . $user->id();

    if ($cache_item = $this->cache->get($cid)) {
      return $cache_item->data;
    }

    $data = $this->loadByUser($user);
  ...
  /**
   * Get current user invitations cache.
   */
  public function cleanUserInvitationsCache($user_id) {
    $cid = GroupInvitationLoaderInterface::GINVITE_USER_INVITATIONS . $user_id;

...
class AddressLookupElement extends FormElementBase {
  ...
  public static function processAddressLookupElement(&$element, FormStateInterface $form_state, &$form): mixed {
    ...
    $is_manual_entry_btn_always_visible = isset($element['#always_display_manual_address_entry_btn']) ? ($element['#always_display_manual_address_entry_btn'] === 'yes') : TRUE;
    $manual_entry_data_attr = $is_manual_entry_btn_always_visible ? 'true' : 'false';
    $element['#prefix'] = '<div id="' . $html_id . '" class="centralhub-address-lookup js-centralhub-address-lookup" data-manual-entry-always-visible="' . $manual_entry_data_attr . '">';
    $element['#suffix'] = '</div>';

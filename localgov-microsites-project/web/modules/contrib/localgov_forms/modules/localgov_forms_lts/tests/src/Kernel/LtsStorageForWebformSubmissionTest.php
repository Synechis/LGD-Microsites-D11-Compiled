...
use Drupal\localgov_forms_test\Driver\Database\FakeLts\Connection as FakeDbConnection;
...
class LtsStorageForWebformSubmissionTest extends KernelTestBase {
  ...
  /**
   * Inserts fake db connection in Database::$connections which is protected.
   *
   * This is a work-around for StubConnection::open()'s return type mismatch
   * issue.  @see #3573167.  Should be reverted once #3573167 is resolved.
   *
   * @see Drupal\Tests\Core\Database\ConnectionTest::testPrefixRoundTrip()
   */
  protected function addFakeDbConnection(): void {

    $reflection_of_database = new \ReflectionClass('Drupal\Core\Database\Database');
    $reflection_of_db_connections = $reflection_of_database->getProperty('connections');

    $stub_pdo = new StubPDO();
    $fake_db_connection = new FakeDbConnection($stub_pdo, []);

    $db_connections = $reflection_of_db_connections->getValue();
    $db_connections[Constants::LTS_DB_KEY]['default'] = $fake_db_connection;
    $reflection_of_db_connections->setValue(NULL, $db_connections);
  }

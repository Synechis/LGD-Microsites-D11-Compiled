...
class GroupContentTypeAccessTest extends BrowserTestBase {
  ...
  protected function setUp(): void {
    ...
    $this->container->get('module_installer')->install(['localgov_microsites_blogs']);
    ...
    foreach ($modules as $module_name => $module_info) {
      ...
      foreach ($modules as $check_content) {
        foreach ($check_content['content_types'] as $check_content_type) {
          ...
          $this->assertEquals($check_content['status'], $status_code, "Testing $check_content_type, expected " . $check_content['status'] . " but got " . $status_code . ")");
    ...
    while (TRUE) {
      ...
      foreach ($modules as $check) {
        foreach ($check['paths'] as $path) {
          ...
          $status_code = $this->getSession()->getStatusCode();
          $this->assertEquals($check['status'], $status_code, "Testing $path , expected " . $check['status'] . " but got " . $status_code);
        }
      }

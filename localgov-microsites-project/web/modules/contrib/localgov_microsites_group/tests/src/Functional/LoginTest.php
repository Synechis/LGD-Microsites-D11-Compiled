...
class LoginTest extends BrowserTestBase {
  ...
  public function testLoginRedirect() {
    ...
    $this->assertSession()->addressEquals(Url::fromRoute('entity.group.canonical', ['group' => 1])->toString());
  }
  ...
  public function testLoginOutTrait() {
    ...
    $this->assertSession()->addressEquals(Url::fromUri('internal:/user')->toString() . '/' . $this->testUser->id());
    $this->drupalGet($domain2->getUrl() . $user_page);
    $this->assertSession()->addressEquals(Url::fromUri('internal:/user')->toString() . '/' . $this->testUser->id());
    $this->drupalGet($domain3->getUrl() . $user_page);
    $this->assertSession()->addressEquals(Url::fromUri('internal:/user')->toString() . '/' . $testUser2->id());

...
class MicrositeWebformAccessTest extends BrowserTestBase {
  ...
  protected function setUp(): void {
    parent::setUp();
    $this->container->get('module_installer')->install(['localgov_microsites_group_webform']);
  ...
  public function testMicrositeNodeWebform() {
    ...
    $this->drupalGet($this->domains[1]->getUrl() . 'node/' . $this->webforms[1]->id());
    ...
    $this->drupalGet($this->domains[1]->getUrl() . 'node/' . $this->webforms[2]->id());
    $this->assertSession()->statusCodeEquals(403);

    // Check form 2 only on domain 2 and make a submission.
    $this->drupalGet($this->domains[2]->getUrl() . 'node/' . $this->webforms[1]->id());
    $this->assertSession()->statusCodeEquals(403);
    $this->drupalGet($this->domains[2]->getUrl() . 'node/' . $this->webforms[2]->id());
    $this->assertSession()->statusCodeEquals(200);

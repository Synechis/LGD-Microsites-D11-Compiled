...
class DomainPathHelper {
  ...
  public function validateEntityForm(array &$form, FormStateInterface $form_state) {
    ...
    $domain_access_all = !empty($form['field_domain_all_affiliates'])
      ? $form_state->getValue('field_domain_all_affiliates')['value'] : FALSE;
    ...
    foreach ($domain_path_values as $domain_id => $domain_path_data) {
      ...
      if (!empty($alias) && !str_starts_with($alias, '/')) {
        // Attempt to get domain label for better messaging.
        $domain_label = $domain_id;
        if (isset($domain_entities[$domain_id])) {
          $domain_label = $domain_entities[$domain_id]->label();
        }

        $form_state->setErrorByName("domain_path][$domain_id][path",
          t('The alias for %domain must start with a slash.', ['%domain' => $domain_label]));
      }
  ...
  public function submitEntityForm($form, FormStateInterface $form_state) {
    ...
    // If domain access is on for this form, we check the "all affiliates"
    // checkbox, otherwise we just assume it's not available on all domains.
    $domain_access_all = !empty($form['field_domain_all_affiliates']) ? $form_state->getValue('field_domain_all_affiliates')['value'] : FALSE;

...
class AttachMediaToGroup {
  ...
  public function assignMediaToGroups(array $media_items, array $groups) {
    ...
    foreach ($media_items as $media_item) {
      ...
      foreach ($groups as $group) {
        ...
        $group_count = $this->getGroupCount($media_item, $group->bundle());
   ...
   */
  private function getMediaGroupRelationshipEnablerPlugin(GroupInterface $group, $plugin_id) {
    ...
    if ($group_type_plugins->has($plugin_id)) {
      $plugin = $group_type_plugins->get($plugin_id);
   ...
   *
   * @param Drupal\Core\Entity\EntityInterface $media_item
   *   Media entity.
   ...
   *   Group count.
   */
  private function getGroupCount(EntityInterface $media_item, string $group_type_id) {
    // Check if it was calculated already.
    if (!isset($this->groupCount[$media_item->id()][$group_type_id])) {
      ...
      foreach ($group_relationships as $group_relationship) {
        if ($group_relationship->getGroupTypeId() !== $group_type_id) {
          continue;
        }
        $group_ids[] = $group_relationship->getGroupId();
      }
      $this->groupCount[$media_item->id()][$group_type_id] = count(array_unique($group_ids));
    }

    return $this->groupCount[$media_item->id()][$group_type_id];
  }

...
As of this release, the `localgov_sa11y` editorial accessibility checker is **no
longer installed or required** by the Microsites profile. It has been removed as
a dependency so that it is no longer baked into the distribution.

Existing sites are unaffected: if `localgov_sa11y` is already installed it will
keep working, and sites may uninstall it when they choose. New installs will not
include it.

The community's recommended replacement is Editoria11y. Adopting it is a
separate piece of work — see the Editoria11y META:
[TODO: add Editoria11y META link before release].
